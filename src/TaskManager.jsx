...
const TaskManager = () => {
  ...
  // Adiciona estado para controlar a visibilidade da descrição
          ...
          {tasks.map((task, index) => (
            <li key={index} >
              ...
              <button className="toggle-description" onClick={() => toggleDescription(index)}>
                {visibleDescriptions[index] ? '👁️' : '👁️'}
              </button>
              {visibleDescriptions[index] && <p className={task.completed ? 'completed' : ''}>{task.description}</p>}
              <button className="complete" onClick={() => completeTask(index)}>Concluir</button>
            </li>
          ))}
        </ul>
      )}
